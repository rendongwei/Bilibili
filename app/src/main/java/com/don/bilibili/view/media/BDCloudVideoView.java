...
package com.don.bilibili.view.media;
...
/**
 * 开源类-播放器视图VideoView 可以作为控件增加到App中，该类的主要逻辑为：
 * 在Jelly_Bean(4.1)及以上系统，使用TextureView控件展示视频内容
 * 在Jelly_Bean(4.0)及以下系统，使用SurfaceView控件来展示视频内容
 */
public class BDCloudVideoView extends FrameLayout implements
		MediaController.MediaPlayerControl {
	private static final String TAG = "BDCloudVideoView";

	public static final int AR_ASPECT_FIT_PARENT = 0; // without clip,
														// 等比例填充，视频的边界小于或等于显示屏边界，可能留有黑边
	public static final int AR_ASPECT_FILL_PARENT = 1; // may
	// clip，等比例填充，视频边界大于等于显示屏边界，不留黑边，一部分视频区域可能在显示屏边界之外
	public static final int AR_ASPECT_WRAP_CONTENT = 2; // 按照视频实际大小播放
	public static final int AR_MATCH_PARENT = 3; // 不按比例，满屏播放
	public static final int AR_16_9_FIT_PARENT = 4; // 16:9,
													// 视频的边界小于或等于显示屏边界，可能留有黑边
	public static final int AR_4_3_FIT_PARENT = 5; // 4:3,
													// 视频的边界小于或等于显示屏边界，可能留有黑边

	/**
	 * 优先使用TextureView 若为true，则在4.1及以上系统，使用TextureView控件展示视频内容，低版本使用SurfaceView；
	 * 若为false，则始终使用SurfaceView控件展示视频内容； 两者对比：TextureView更耗性能，但支持动画、截图等功能。
	 */
	...
	public enum PlayerState {
		STATE_ERROR(-1), STATE_IDLE(0), STATE_PREPARING(1), STATE_PREPARED(2), STATE_PLAYING(
				3), STATE_PAUSED(4), STATE_PLAYBACK_COMPLETED(5);
	...
	private void addCachingHintView() {
		...
		LayoutParams fllp = new LayoutParams(
		...
		cachingProgressBar.setId(android.R.id.text1); // setId() param can be
														// random number, use
														// text1 to avoid lints
	...
	/**
	 * Sets specific headers. 需在setVideoPath之前调用
	 * 
	 * @param headers
	...
	public BDCloudMediaPlayer createPlayer() {
		BDCloudMediaPlayer bdCloudMediaPlayer = new BDCloudMediaPlayer(
				this.getContext());
		...
		if (mUseApmDetect) {
			bdCloudMediaPlayer.setUseApmDetect(mUseApmDetect); // 开启APM检测，开启时需要额外嵌入APM
																// SDK。
		}
	...
	/**
	 * 设置"加载中"触发时，缓存多长时间的数据才结束 * 注意：若mMediaPlayer为null时，实际上会在createPlayer()中设置，
	 * 这是mCacheTimeInMilliSeconds成员在此赋值的原因；
	 * 
	 * @param milliSeconds
	 */
	...
	/**
	 * 设置最大探测时长 类似于老接口 setFirstBufferingTime
	 * 
	...
	/**
	 * 设置初始播放位置 注意：若mMediaPlayer为null时，实际上会在createPlayer()中设置，
	 * 这是mInitPlayPositionInMilliSec成员在此赋值的原因；
	 * 
	 * @param milliSeconds
	 */
	 ...
	 * 
	 * @param decodeMode
	 *            DECODE_AUTO(优先硬解，找不到硬解解码器则软解)或DECODE_SW(软解)
	 */
	...
	IMediaPlayer.OnVideoSizeChangedListener mSizeChangedListener = new IMediaPlayer.OnVideoSizeChangedListener() {
		public void onVideoSizeChanged(IMediaPlayer mp, int width, int height,
			...
			Log.d(TAG, "onVideoSizeChanged width=" + width + ";height="
					+ height + ";sarNum=" + sarNum + ";sarDen=" + sarDen);
	...
	IMediaPlayer.OnPreparedListener mPreparedListener = new IMediaPlayer.OnPreparedListener() {
		public void onPrepared(IMediaPlayer mp) {
			...
			Log.d(TAG, "onPrepared: mVideoWidth=" + mVideoWidth
					+ ";mVideoHeight=" + mVideoHeight + ";mSurfaceWidth="
	...
	public void start() {
		if (mMediaPlayer != null && (mCurrentState == PlayerState.STATE_ERROR)
			...
			// if your link is not live link, you can comment the following if
			// block
			...
			mMediaPlayer.prepareAsync(); // will start() in onPrepared, because
											// isTryToPlaying = true
			sendCachingHintViewVisibilityMessage(true);
			setCurrentState(PlayerState.STATE_PREPARING);
	...
	private boolean isInPlaybackState() {
		return (mMediaPlayer != null
				...
				&& mCurrentState != PlayerState.STATE_IDLE && mCurrentState != PlayerState.STATE_PREPARING);
	}
	...
	public void setVideoScalingMode(int mode) {
		if (mode == AR_16_9_FIT_PARENT || mode == AR_4_3_FIT_PARENT
				|| mode == AR_ASPECT_FILL_PARENT
				|| mode == AR_ASPECT_FIT_PARENT
				|| mode == AR_ASPECT_WRAP_CONTENT || mode == AR_MATCH_PARENT) {
			mCurrentAspectRatio = mode;
			if (mRenderView != null) {
	 ...
	 * mMediaPlayer.selectResolutionByIndex函数处理过程中，mediaplayer的内部逻辑会stop
	 * -->选择码率-->prepareAsync 当处理完成后，您会收到一个onPrepared事件
	 * 
	 * @param index
	 *            该index为getVariantInfo()数组的合法下标
	 */
	 ...
	 * 获取截图接口
	 * 
	 * 截图原理： TextureView系统接口支持getBitmap截图 SurfaceView暂不支持截图(影响4.0及以下系统的用户)
	 */
	...
	/**
	 * tell BDCloudVideoView that activity will be put into background should
	 * invoke before super.onStop() in an activity avoid video frame blocked in
	 * small probability on some device when using TextureRenderView
	 */
	...
	/**
	 * tell BDCloudVideoView that activity will come back to foreground avoid
	 * video frame blocked in small probability on flyme device when using
	 * TextureRenderView
	 */
